...
const POLL_MS      = 2_000;   // normal polling interval
const POLL_MS_SLOW = 5_000;   // degraded polling interval (error / disconnected)
const SIM_DELAY    = 500;
...
/** An alert generated by fleet state transitions. */
export interface FleetAlert {
  id: string;
  type: 'offline' | 'battery_low' | 'error_state';
  robotName: string;
  message: string;
  timestamp: number;
}
...
export interface UseFleetGatewayReturn {
  ...
  /** Active fleet alerts (offline, battery_low, error_state). */
  alerts: FleetAlert[];
...
export function useFleetGateway(simMode: boolean): UseFleetGatewayReturn {
  ...
  // Refs for alert detection
  const prevRobotsRef     = useRef<GQLRobot[]>([]);
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      // ── Alert detection ────────────────────────────────────────────────
      const prev = prevRobotsRef.current;
      const newAlerts: FleetAlert[] = [];

      fetched.forEach(robot => {
        const prevRobot = prev.find(r => r.name === robot.name);

        // Online → Offline transition
        if (
          prevRobot &&
          prevRobot.connectionStatus === 'ONLINE' &&
          robot.connectionStatus === 'OFFLINE'
        ) {
          newAlerts.push({
            id: `offline-${robot.name}-${Date.now()}`,
            type: 'offline',
            robotName: robot.name,
            message: `${robot.name} has gone OFFLINE`,
            timestamp: Date.now(),
          });
        }

        // Battery low (below 20, not already alerted)
        // NOTE: GQLRobot doesn't carry battery in its GraphQL response, so we
        // only process this if a battery field exists (future-proof).
        const batteryAny = (robot as any).battery as number | undefined;
        if (batteryAny !== undefined) {
          if (batteryAny < 20 && !batteryAlertedRef.current.has(robot.name)) {
            batteryAlertedRef.current.add(robot.name);
            newAlerts.push({
              id: `battery_low-${robot.name}-${Date.now()}`,
              type: 'battery_low',
              robotName: robot.name,
              message: `${robot.name} battery low: ${batteryAny}%`,
              timestamp: Date.now(),
            });
          } else if (batteryAny > 30) {
            batteryAlertedRef.current.delete(robot.name);
          }
        }
        ...
        if (
          prevRobot &&
          prevRobot.lastActionStatus !== 'ERROR' &&
          robot.lastActionStatus === 'ERROR'
        ) {
          newAlerts.push({
            id: `error_state-${robot.name}-${Date.now()}`,
            type: 'error_state',
            robotName: robot.name,
            message: `${robot.name} entered ERROR state`,
            timestamp: Date.now(),
          });
        }
      });
  ...
  useEffect(() => {
    ...
    const scheduleNext = () => {
      const interval = connectedRef.current ? POLL_MS : POLL_MS_SLOW;
    ...
    const onVisibilityChange = () => {
      if (!document.hidden) {
        // Page became visible — cancel pending timer and poll immediately
        if (timerId !== null) clearTimeout(timerId);
        void refresh().then(() => scheduleNext());
      } else {
        // Page hidden — cancel pending timer (will restart on visibility change)
        if (timerId !== null) {
  ...
  return { robots, connected, pollError, refresh, activeRobotName, setActiveRobotName, dispatchRequest, hardReset, alerts, dismissAlert, clearAlerts };
}
